const stringEcho = (arg: string): string => arg // Only works with a string type
const Echo = <T>(arg: T): T => arg // Works with any type
...
const chockBoolValue = <T>(arg: T): BoolCheck<T> => {
    if (Array.isArray(arg) && !arg.length) {
